...
import static com.example.projectver3.HomeActivity.DocDL;
...
//                        DocDLDM();
                        ...
                        Toast.makeText(AddUpdateDanhMucActivity.this, "Danh mục đang được sử dụng", Toast.LENGTH_SHORT).show();
                    }
                }
            });
        } else {
...
//        imgColor.setBackgroundColor(Color.parseColor(temp.getMau()));
    ...
    public static boolean checkGD(List<GiaoDich> listGD, DanhMuc dm){

        for (GiaoDich gd : listGD) {
            if (gd.getDanhMuc().getMaDanhMuc() == dm.getMaDanhMuc()){
                return false;
            }
        }
        return true;
